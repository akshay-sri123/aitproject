...
public class Test {
	...
	public void test1() throws IOException, IllegalAccessException, NoSuchFieldException {
		...
		metricsList.add(new AggregationMetrics(parameters, AggregationTypes.SUM));
		...
		while((nextLine = csvReader.readNext()) != null)
		{
			...
			if(nextLine != null)
			{
				...
				{
					int keyLen = length.getByteLength(schema.rowSchema, adInfo);
					
					Row rowEntry = new Row();
					
					rowEntry.dataBytes = new byte[keyLen];
					rowEntry = coder.encoder(schema.rowSchema, adInfo);
					
					resultMap = operations.operations(rowEntry, metricsList.get(0), schema, resultMap);
				}
			}
		}
...
//		}
	...
	public void test2() throws NoSuchFieldException, IllegalAccessException {
		...
		String keys[] = {"publisher","location","clicks"};
		List<AggregationMetrics> metricsList = new ArrayList<>();
	...
	public void test3() throws IOException, NoSuchFieldException, IllegalAccessException {
		...
		String parameters[] = {"advertiser","cost","impressions"};
		
		List<AggregationMetrics> metricsList = new ArrayList<>();
		metricsList.add(new AggregationMetrics(parameters, AggregationTypes.MAX));
		...
		for (AggregationSchema schema : schemaList)
		{
			...
			rowEntry.dataBytes = new byte[keyLen];
			rowEntry = coder.encoder(schema.rowSchema, adInfo);
			System.out.println(Arrays.toString(rowEntry.getDataBytes()));
			
			adInfo1 = (AdInfo) coder.decoder(schema.rowSchema, rowEntry, adInfo1);
...
////			int key Len = length.getByteLength(schema.rowSchema, adInfo);
////
////			Row keyRow = new Row();
////
